...
// Service_2 - 10 Hz, every 5th Sequencer loop 
...
void Sequencer(int id)
{
    ...
    // Servcie_1 = RT_MAX-1	@ 50 Hz @ Every 2 ticks
   if((seqCnt % 2) == 0) sem_post(&semS1);

    // Service_2 = RT_MAX-2	@ 10 Hz @ Every 10 ticks
   if((seqCnt % 10) == 0) sem_post(&semS2);

    // Service_3 = RT_MAX-3	@ 6.67 Hz @ Every 15 ticks
...
void *Service_3(void *threadp)
{
    ...
    while(!abortS3)
    {
        ...
        clock_gettime(MY_CLOCK_TYPE, &current_time_val); current_realtime=realtime(&current_time_val);
        syslog(LOG_CRIT, "S3 6.67 Hz on core %d forrelease %llu @ sec=%6.9lf\n", sched_getcpu(), S3Cnt, current_realtime-start_realtime);
    }
