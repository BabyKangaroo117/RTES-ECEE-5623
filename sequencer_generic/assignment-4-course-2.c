...
// Service_3 - 14.3 Hz    every 7th Sequencer loop
// Service_4 - 7.7  Hz    every 13th Sequencer loop
// With the above, priorities by RM policy would be:
//
// Sequencer = RT_MAX	@ 100   Hz
// Servcie_1 = RT_MAX-1	@ 50    Hz
// Service_2 = RT_MAX-2	@ 20    Hz
// Service_3 = RT_MAX-3	@ 14.4  Hz
// Service_4 = RT_MAX-4 @ 7.7   HZ
/////////////////////////////////////////////////////////////////////////////
// JETSON SYSTEM NOTES:
/////////////////////////////////////////////////////////////////////////////
//
...
int main(void)
{
    ...
    if (fp == NULL)
    {
       syslog(LOG_ERR, "[COURSE:2][ASSIGNMENT:4]: Failed to run uname command");
       return 1;
    }
    ...
    {
       syslog(LOG_CRIT, "[COURSE:2][ASSIGNMENT:4]: %s", buffer);
    }
...
void *Service_1(void *threadp)
{
	...
	// wait for service request from the sequencer, a signal handler or ISR in kernel
        ...
        clock_gettime(MY_CLOCK_TYPE, &current_time_val); current_realtime=realtime(&current_time_val);
        syslog(LOG_CRIT, "[COURSE:2][ASSIGNMENT:4]: Thread 1 start %llu @ sec=%6.9lf on core %d\n", S1Cnt, current_realtime-start_realtime, sched_getcpu());
	...
	S1Cnt++;
        ...
        printf("[COURSE:2][ASSIGNMENT:4]: Thread 1 took %6.9lf Msec \n", end_time - start_time);
    }
...
void *Service_2(void *threadp)
{
	...
	// wait for service request from the sequencer, a signal handler or ISR in kernel
        ...
        start_time = getTimeMsec();
        clock_gettime(MY_CLOCK_TYPE, &current_time_val); current_realtime=realtime(&current_time_val);
        syslog(LOG_CRIT, "[COURSE:2][ASSIGNMENT:4]: Thread 2 start %llu @ sec=%6.9lf on core %d\n", S2Cnt, current_realtime-start_realtime, sched_getcpu());
	...
	S2Cnt++;
        ...
        end_time = getTimeMsec();
        printf("[COURSE:2][ASSIGNMENT:4]: Thread 2 took %6.9lf Msec \n", end_time - start_time);
...
void *Service_3(void *threadp)
{
	...
	// wait for service request from the sequencer, a signal handler or ISR in kernel
        ...
        start_time = getTimeMsec();
        clock_gettime(MY_CLOCK_TYPE, &current_time_val); current_realtime=realtime(&current_time_val);
        syslog(LOG_CRIT, "[COURSE:2][ASSIGNMENT:4]: Thread 3 start %llu @ sec=%6.9lf on core %d\n", S3Cnt, current_realtime-start_realtime, sched_getcpu());
	...
	S3Cnt++;
        ...
        end_time = getTimeMsec();
        printf("[COURSE:2][ASSIGNMENT:4]: Thread 3 took %6.9lf Msec \n", end_time - start_time);
...
void *Service_4(void *threadp)
{
	...
	// wait for service request from the sequencer, a signal handler or ISR in kernel
        ...
        start_time = getTimeMsec();
        clock_gettime(MY_CLOCK_TYPE, &current_time_val); current_realtime=realtime(&current_time_val);
        syslog(LOG_CRIT, "[COURSE:2][ASSIGNMENT:4]: Thread 4 start %llu @ sec=%6.9lf on core %d\n", S4Cnt, current_realtime-start_realtime, sched_getcpu());
	...
	S4Cnt++;
        ...
        end_time = getTimeMsec();
        printf("[COURSE:2][ASSIGNMENT:4]: Thread 4 took %6.9lf Msec \n", end_time - start_time);
