...
int main(void)
{
    ...
    if (fp == NULL)
    {
       syslog(LOG_ERR, "[COURSE:2][ASSIGNMENT:6]: Failed to run uname command");
       return 1;
    }
    ...
    {
       syslog(LOG_CRIT, "[COURSE:2][ASSIGNMENT:6]: %s", buffer);
    }
...
void *Service_1(void *threadp)
{
	...
	// wait for service request from the sequencer, a signal handler or ISR in kernel
        ...
        clock_gettime(MY_CLOCK_TYPE, &current_time_val); current_realtime=realtime(&current_time_val);
        syslog(LOG_CRIT, "[COURSE:2][ASSIGNMENT:6]: Thread 1 start %llu @ sec=%6.9lf on core %d\n", S1Cnt, current_realtime-start_realtime, sched_getcpu());
	...
	S1Cnt++;
        ...
        printf("[COURSE:2][ASSIGNMENT:6]: Thread 1 took %6.9lf Msec \n", end_time - start_time);
    }
...
void *Service_2(void *threadp)
{
	...
	// wait for service request from the sequencer, a signal handler or ISR in kernel
        ...
        start_time = getTimeMsec();
        clock_gettime(MY_CLOCK_TYPE, &current_time_val); current_realtime=realtime(&current_time_val);
        syslog(LOG_CRIT, "[COURSE:2][ASSIGNMENT:6]: Thread 2 start %llu @ sec=%6.9lf on core %d\n", S2Cnt, current_realtime-start_realtime, sched_getcpu());
	...
	S2Cnt++;
        ...
        end_time = getTimeMsec();
        printf("[COURSE:2][ASSIGNMENT:6]: Thread 2 took %6.9lf Msec \n", end_time - start_time);
...
void *Service_3(void *threadp)
{
	...
	// wait for service request from the sequencer, a signal handler or ISR in kernel
        ...
        start_time = getTimeMsec();
        clock_gettime(MY_CLOCK_TYPE, &current_time_val); current_realtime=realtime(&current_time_val);
        syslog(LOG_CRIT, "[COURSE:2][ASSIGNMENT:6]: Thread 3 start %llu @ sec=%6.9lf on core %d\n", S3Cnt, current_realtime-start_realtime, sched_getcpu());
	...
	S3Cnt++;
        ...
        end_time = getTimeMsec();
        printf("[COURSE:2][ASSIGNMENT:6]: Thread 3 took %6.9lf Msec \n", end_time - start_time);
...
void *Service_4(void *threadp)
{
	...
	// wait for service request from the sequencer, a signal handler or ISR in kernel
        ...
        start_time = getTimeMsec();
        clock_gettime(MY_CLOCK_TYPE, &current_time_val); current_realtime=realtime(&current_time_val);
        syslog(LOG_CRIT, "[COURSE:2][ASSIGNMENT:6]: Thread 4 start %llu @ sec=%6.9lf on core %d\n", S4Cnt, current_realtime-start_realtime, sched_getcpu());
	...
	S4Cnt++;
        ...
        end_time = getTimeMsec();
        printf("[COURSE:2][ASSIGNMENT:6]: Thread 4 took %6.9lf Msec \n", end_time - start_time);
