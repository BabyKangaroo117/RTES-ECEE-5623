...
void *ReadImageService(void *threadp);
void *DumpImageService(void *threadp);
...
static int read_frame(void)
{
    ...
    switch (io)
    {
        ...
        case IO_METHOD_MMAP:
            ...
            frameInfo.index = buf.index;
	    frameInfo.bytesused = buf.bytesused;
            printf("Send %d \n", sizeof(frameInfo)); 
	    if(mq_send(mq, (char *)&frameInfo, sizeof(frameInfo), 0) == -1) {
		 perror("mq_send");
	    }
...
void *DumpImageService(void *threadp)
{
    ...
    int count = 0; 
     while (count < frame_count) 
     {
        ...
        printf("Dump %d \n", sizeof(frameInfo));
        if (mq_receive(mq, (char*)&frameInfo, sizeof(frameInfo), NULL) == -1)
        {
	    perror("mq_receive");
	    continue;
        }
        
        process_image(buffers[frameInfo.index].start, frameInfo.bytesused);
        ...
        qbuf.index = frameInfo.index;

        if (-1 == xioctl(fd, VIDIOC_QBUF, &qbuf)) {
           errno_exit("VIDIOC_QBUF");
        }
	count++;
     }
...
int main(int argc, char **argv)
{
    ...
    attr.mq_curmsgs = 0;
    printf("Initial %d \n", sizeof(FrameInfo));
    mq_unlink("/frame_queue");
    mq = mq_open("/frame_queue", O_CREAT | O_RDWR, 0644, &attr);
    if (mq == (mqd_t)-1) {
	 perror("mq_open");
	 exit(1);
    }
    ...
    int rt_max_prio, rt_min_prio, cpuidx, rc, scope;
   ...
   printf("Using CPUS=%d from total available.\n", CPU_COUNT(&allcpuset));
    ...
    rc=pthread_create(&threads[0],               // pointer to thread descriptor
                      ...
                      ReadImageService,                 // thread function entry point
    ...
    rc=pthread_create(&threads[1], &rt_sched_attr[1], DumpImageService, (void *)&(threadParams[1]));
